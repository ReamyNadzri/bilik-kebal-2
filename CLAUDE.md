...
## Two-lane delivery: Codex backend, Claude frontend
...
`workers/**`. **Claude Code** owns `codex/provisional-ui`: `src/app/**` pages and layouts (not
...
`Owner: Claude Code`. Codex publishes contracts first; consume them rather than forking domain types,
...
Claude Code owns a **complete provisional frontend** across all MVP routes, flows, responsive
